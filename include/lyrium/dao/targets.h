...
inline constexpr Target targets[] = {
    ...
    // Recovered with Ghidra rather than inherited, and produced by
    // tools/ghidra/emit_target_rows.py -- which reproduces all thirteen rows above
    // byte for byte, including every reloc_mask, when pointed at the same binary.
    {
        .name = "pool_alloc",
        .symbol = "sub_4B92C0",
