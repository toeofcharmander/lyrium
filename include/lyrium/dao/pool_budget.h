...
// actual_bytes is the size the engine registered, which is what a detour on the
// registrar reads. It is never larger than expected in practice, but it arrives
// from engine memory, so a surprising value must not wrap the subtraction.
[[nodiscard]] constexpr auto evaluate_main_pool(std::uint64_t budget_bytes, std::uint64_t actual_bytes)
    -> MainPoolOutcome
{
    const auto expected = expected_main_pool_bytes(budget_bytes);
    const auto shortfall = actual_bytes < expected ? expected - actual_bytes : std::uint64_t{0};
